   ...
   for(i=0;i<deals;i++)
    {
     ...
     if(HistoryDealGetInteger(tic,DEAL_TYPE) == DEAL_TYPE_BUY ) {
         type_obj = OBJ_ARROW_BUY;
         txt = " BUY ";
     }
     else if(HistoryDealGetInteger(tic,DEAL_TYPE) == DEAL_TYPE_SELL ){ 
         type_obj = OBJ_ARROW_SELL; 
         txt = " SELL ";
     }
     //----
       ...
       for(int ii=0;ii<deals;ii++)
        {
         ...
         if( HistoryDealGetInteger(deal_ticket,DEAL_POSITION_ID) == pos_id && HistoryDealGetInteger(deal_ticket,DEAL_ENTRY) == DEAL_ENTRY_OUT )
          { 
            if(HistoryDealGetDouble(deal_ticket,DEAL_PROFIT)<0)
               colir = Red;
            else{
               colir = Green; 
               coun2++;
